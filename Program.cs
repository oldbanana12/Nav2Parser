...
namespace Nav2Parser
{
    class Program
    {
        static void Main(string[] args)
        {
            if (args.Length > 0)
            {
                var fi = File.GetAttributes(args[0]);
                ...
                else if (fi.HasFlag(FileAttributes.Directory))
                {
                    var di = new DirectoryInfo(args[0]);
                    FileInfo[] files = null;
                    files = di.GetFiles("*.nav2");
                    if (files != null)
                    {
                        Console.WriteLine("Fullname, X, Y, Z, u1e, u1f, u1g, u1h");
                        foreach (var file in files)
                        {
                            var nav2 = new Nav2();
                            nav2.Read(file.FullName);
                            Console.WriteLine("{0},{1},{2},{3},{4},{5},{6},{7}", file.FullName, nav2.header.origin.x, nav2.header.origin.y, nav2.header.origin.z, nav2.header.xDivisor, nav2.header.yDivisor, nav2.header.zDivisor, nav2.header.u1h);
                        }
                    }
                } 
                else
